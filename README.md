...
| 🟢 [xAI (official)](https://x.ai) | official-relay | Card | Vendor's own list price for Grok (text + Grok Imagine image/video), scraped as the official baseline that relays like Atlas Cloud resell. Not a relay station — included only as a price reference. |
| 🟢 [Atlas Cloud](https://www.atlascloud.ai) | aggregator | Card | Multi-modal aggregator; image/video heavy (Grok Imagine, Kling, ByteDance, Vidu). The /v1/models JSON only lists ~120 text LLMs at a single price; the pricing page RSC exposes all 300+ models with list-vs-discounted prices, so the fetcher reads that instead. |
